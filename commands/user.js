exports.run = async (client, message, args) => { 
    ...
    let profile = `https://ryouji.glitch.me/user/${user.id}`;
     ...
     case 'desc': {
      let text = args.join(' ')
       db.fetch(`userDesc_${user.id}`).then(desc => {
           if(desc == null) return message.author('Você não possui o item "Descrição". Digite: **r!buy Desc** para comprar esse item e ter direito de alterar isso!');
       });  
        ...
        if(txt.length >= 150) return message.reply('Sua descrição possui mais de 150 caractéres!')
         ...
         let Embed = new Discord.RichEmbed()
             .setTitle('<:green:463073006093336576> Alterado com Sucesso!')
             .setColor(client.color)
